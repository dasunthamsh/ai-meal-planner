...
ga.scaler = scaler  # inject scaler for denormalization
...
def calculate_target_nutrition(user_data):
    ...
    return {
        'calories': calories,
        'protein': (calories * p_ratio) / 4,
        'fat': (calories * f_ratio) / 9,
        'carbs': (calories * c_ratio) / 4
    }
...
def generate_meal_plan():
    try:
        ...
        # Build response with accurate denormalized values
        meal_plan = []
        for day_idx, day_meals in enumerate(best_solution):
            ...
            for meal_type, meal_idx in day_meals.items():
                ...
                cal = nutrition_dict['calories']
                pro = nutrition_dict['protein']
                fat = nutrition_dict['fat']
                carb = nutrition_dict['carbs']
                ...
                meal_data = {
                    ...
                    'sodium': round(nutrition_dict.get('sodium_mg', 0), 1),
                    'sugar': round(nutrition_dict.get('sugar_g', 0), 1),
                    'fiber': round(nutrition_dict.get('fiber_g', 0), 1),
                    'cholesterol': round(nutrition_dict.get('cholesterol_mg', 0), 1)
                }
